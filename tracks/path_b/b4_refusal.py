...
def run_b4(
    ...
    resolved_config = config_path or B4_CONFIG_PATH
    role_model_map = load_config(resolved_config)
    config_tag = resolved_config.stem
    meta = B4_META[scenario_slug]

    print(
        f"[B4] {session_id} | scenario={scenario_slug} | config={config_tag} | type={meta['type']} | expected={meta['expected_verdict']}",
        flush=True,
    )
    ...
    out_dir = B4_OUTPUT_DIR if config_tag == "b3_b4_baseline" else B4_OUTPUT_DIR / config_tag
    out_dir.mkdir(parents=True, exist_ok=True)
    b4_json = out_dir / f"{_timestamp()}_{scenario_slug}.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to a config YAML (overrides default b3_b4_baseline). "
             "Output goes to output/b4/<config_stem>/",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.batch:
        ...
        for scenario_slug in B4_SCENARIOS:
            ...
            if not scenario_path.exists():
                ...
                continue
            results.append(run_b4(scenario_slug, scenario_path, args.quiet, config_path))
    ...
    run_b4(args.scenario, scenario_path, args.quiet, config_path)
